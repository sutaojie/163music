{
    let view = {
        ...
        template:`
            <form class="form">
                ...
                <div class="row">
                    ...
                    <input name="singer" type="text" value="__songer__">
                </div>
                <div class="row">
                    <label>
                        外链
        ...
        render(data = {}) {   // es6语法，如果用户没有传data或者data是undefined,默认执行data等于空对象
            let placeholders = ['name', 'songer', 'url', 'id','cover','lyrics','number']
    ...
    let model = {
        data:{
            name:'', songer:'', url:'', id:'',cover:'',lyrics:'',number:''
        },
        update(data){
    ...
    let controller = {
        ...
        create(){
            let needs = 'name songer url cover lyrics number'.split(' ')
            let data ={}
            needs.map((string)=>{
